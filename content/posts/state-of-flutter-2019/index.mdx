...
**# Flutter has managed to gain a huge following from the community in  the short time** it's been around it has grown to be the most active topic on StackOverflow and the highest growing skill on linked in.
  ...
  ### The bad
    ...
    **# The syntax is just the insane amount of nesting** that goes around in making the widgets or "screens" so to say.It's okay
    ...
    **# The promise of flutter working on web and desktop platforms is still a work in progress🚧**,I know you're like "what are you trying to say here", I have to admit yes I am a fan of flutter but  for now 
  I would advise to use in mobile applications where it just slays with its possitives that I already talked about, but for the web and desktop implementations although making really good 
  ...
  **# Flutter does not deal with any native  components it means that all the IOS and Android material design components have to be implemented by the Flutter team & the community** and then packaged for use in the development
  ...
  flutter apps this change would not be reflected until the next time the flutter team or the community updates the component or widget so to say to match the current updates also the behaviour of flutter components might not always
  ...
  and the current implementation work well enough. Most companies use custom design sytems anyways so it shouldn't be much of a problem.
...
playstore(Code push) I have to admit this is one of the features that I miss the most. This feature was on Flutter teams roadmap for 2019 but has then been put on pause because of reasons explained [here]()
.
